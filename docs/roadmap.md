...
- ~~No transactions (`MULTI`/`EXEC`)~~ **Done (2026-09-10).** With
  `WATCH` for optimistic locking. See [transactions.md](transactions.md).
...
9. **Pub/sub, then blocking commands** - now the top of the list. Both
   need the event loop to park and wake a connection, the one piece
   neither RESP nor transactions brought.
